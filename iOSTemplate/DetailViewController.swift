...
//      displayPlaybackButton()
  ...
  func displaySubscriptionOptions(_ sender: UIButton) {
    Session.shared().getProperty({ (property, error) in
        if (error == nil) {
          Product.query(self.appDelegate.appConfiguration["DEALER_REALM_UUID"] as! String, categories: [], match: self.artifact.id.stringValue, onCompletion: { (products, error) in
